...
const StrokeText = ({
  ...
  useEffect(() => {
    if (fontSizeMobile == null || typeof window === 'undefined') return undefined;
  ...
  const activeFontSize = isMobile && fontSizeMobile != null ? fontSizeMobile : fontSize;
  ...
  const dash = Math.max(activeFontSize * 7, 200);
  ...
  const fontStyle = useMemo(
    ...
    [activeFontSize, fontWeight, letterSpacing]
  );
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const pad = Math.max(Number(strokeWidth) || 1, activeFontSize * 0.1);
      const next = {
        x: bbox.x - pad,
        y: bbox.y - pad,
  ...
  }, [characters, activeFontSize, fontWeight, letterSpacing, strokeWidth]);
  ...
  const viewBox = box ? `${box.x} ${box.y} ${box.width} ${box.height}` : `0 ${-activeFontSize} 600 ${activeFontSize * 1.3}`;
  ...
  return (
    ...
    >
      <svg
        ...
        style={{ height: `${Math.round(activeFontSize * 1.3)}px` }}
        viewBox={viewBox}
